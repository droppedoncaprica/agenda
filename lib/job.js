...
Job.prototype.toJSON = function() { // create a persistable Mongo object -RR
    ...
    var dates = ['lastRunAt', 'lastFinishedAt', 'nextRunAt', 'failedAt', 'lockedAt', 'startAt', 'endAt'];
    dates.forEach(function(d) {
      var isDate = !isNaN(new Date(result[d]));
      if (result[d] && isDate) {
...
Job.prototype.computeNextRunAt = function() {
  ...
  function computeFromInterval() {
    ...
    try {
      ...
      if (this.startAt && nextDate.valueOf() < this.startAt.valueOf()) {
        // Handle startDate being in the future.
        nextDate = cronTime._getNextDateFrom(this.startAt);
      } else if (this.endAt && nextDate.valueOf() > this.endDate()) {
        // Handle end date being passed.
        this.disable();
